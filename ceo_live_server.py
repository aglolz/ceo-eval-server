"""
CEO Voice Coach — LIVE PRODUCTION eval server.

This is the instance the live number routes through (formerly ankita_server.py,
renamed 2026-08-11 — "Ankita's test" had been carrying real production traffic
since the 2026-07-28 live-number swap). It answers assistant-request for the
50/50 A/B split, scores every end-of-call report with all judges, mirrors the
report to the Zapier->Twilio SMS hook so participants get their summary text,
runs the post-call feedback survey, and serves the token-gated /dashboard
boards.

Writes results to Supabase table: ceo_live_calls
(formerly ankita_test_calls — renamed by migrations/011_rename_to_ceo_live_calls.sql)

Deploy to Railway:
  - Procfile points here (gunicorn ceo_live_server:app)
  - Run migrations/011 in Supabase BEFORE pushing (Railway auto-redeploys)
  - If the Railway service sets SUPABASE_TABLE explicitly, update it to
    ceo_live_calls (or unset it to use the default)
"""
